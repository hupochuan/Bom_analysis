...
import indi.ycl.dao.SupplyRelationsDao;
...
public class GetEntity {
	public static void GetSupplyRelation(int id,String company,StanfordNer ner){
		List<String> sentences=new AnnualReportDao().getReportById(id);
...
//		sentences.add("车辆减震产品获得了华晨汽车等工厂认可并获得整车悬置件的订单，并有十多个车型的减震产品正在开发研制，汽车密封件整车业务全面推进，并逐步进入上汽荣威、上汽大通、一汽轿车、一汽大众、广汽三菱、北汽及新能源车的整车密封供系统， 为公司的持续发展打下了坚实基础。");
		...
		for (int i = 0; i < sentences.size(); i++) {
		    ...
		    if(sent.getHasCom()&&sent.getHasPro()){
		    	re1=GetSupplyRelation.extractSupplyRelation(sent, company);
		    	for (int j = 0; j < re1.size(); j++) {
		    		SupplyRelation out=re1.get(j);
		    		sdao.InsertSupplyRelation(out, id);
		    		//System.out.println(out.getClient()+" "+out.getCompany()+" "+out.getProduct());
				}
		    }
		    if(sent.getHasCom()&&!sent.getHasPro()){
		    	re2=GetSupplyRelation.extractClient(sent, company);
	...
	public static void main(String[] args) {
        ...
        ArrayList<Integer> coms=new CompanyDao().GetCompanyByIndustry();
        CompanyDao cdao=new CompanyDao();
        AnnualReportDao adao=new AnnualReportDao();
        StanfordNer ner = new StanfordNer();
        for (int i = 0; i < coms.size(); i++) {
			
			int j = 8517;
			while (j <= 18172) {
				Integer company_id=adao.ExistReport(j,coms);

				if (company_id != null) {
					String company=cdao.GetCompanyName(company_id);
					GetEntity.GetSupplyRelation(j, company,ner);
				}
				j++;
			}
